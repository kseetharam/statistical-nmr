...
def optimize_with_bfgs(
    ...
    n_td: int, sw: int, phase: float, gt: jnp.ndarray, calc_hamiltonian_f: Callable,
    model: NeuralODE, nn_off: bool, filter_spec=None
) -> tuple[jax.Array, NeuralODE]:
    """Optimizes the model using BFGS from SciPy."""

    # Partition the model if needed
    if filter_spec is not None:
        def loss_fn(params):
            # updated_model = eqx.combine(diff_model, params)
            diff_model, static_model = eqx.partition(model, filter_spec)
            updated_model = eqx.tree_at(
                where=lambda m: m.h_mat,
                pytree=diff_model,
                replace=params.reshape(diff_model.h_mat.shape)
            )
            return mse_with_partition(
                updated_model, static_model,
                calc_hamiltonian_f=calc_hamiltonian_f,
                rho=rho, ts=ts, op=op, t2=t2, apodize=apodize, n_td=n_td,
                sw=sw, phase=phase, gt=gt, nn_off=nn_off
            )  # Extract loss value

    else:
        def loss_fn(model):
            # updated_model = eqx.combine(model, params)
            return mse(model[0], rho=rho, ts=ts, gt=gt, nn_off=nn_off)

    # Compute gradients
    # loss_and_grad = jax.value_and_grad(loss_fn)

    def scipy_loss_and_grad(params):
        loss, grads = loss_fn(params)
        return float(loss)

    # Convert model parameters into a flat array
    initial_params = model.h_mat.ravel()

    # Run BFGS optimization
    options = {'disp': True, 'gtol': 1e-8, 'maxiter': 1000, 'ftol':1e-8}
    result = minimize(scipy_loss_and_grad, initial_params, method="BFGS", options=options)

    # Convert optimized parameters back to the model
    # optimized_params = unravel_fn(result.x)
    # optimized_model = eqx.combine(model, optimized_params)

    optimized_model = eqx.tree_at(
        where=lambda m: m.h_mat,
        pytree=model,
        replace=jnp.asarray(result.x.reshape(3,3))
    )

    return result.fun, optimized_model  # Return the final loss and optimized model
...
if __name__ == "__main__":
    ...
    # sd_values = np.linspace(start=0.1, stop=0.2, num=21)[1:-5]
    sd_values = [0.1, 0.2, 0.3]
    for sd in sd_values:

        sd = round(sd, 3)
        logger.info(f"Starting experiment with SD={sd}")
    # sd = 0.105
        h_mat = jnp.triu(
            gt_h_mat + jr.normal(key=key, shape=(n_spins, n_spins)) * sd
        ...
        gt_fid = explicit_exponentiation(hamiltonian=calc_hamiltonian(h_mat=gt_h_mat, B0=B0, Ix=Ix, Iy=Iy, Iz=Iz), rho=rho, op=OP, ts=ts, dt=dt, n_td=n_td, t2=t2, apodize=apodize)
        ...
        n_epochs = {
            # "Warm-up": {"num_steps": 50, "nn_off": False, "h_mat_off": False},
            "Hamiltonian": {"num_steps": 1, "nn_off": True, "h_mat_off": False},
            # "Neural Net": {"num_steps": 50, "nn_off": False, "h_mat_off": True},
        }
        total_epochs = 12
            ...
            for phase_name, phase_config in n_epochs.items():
                ...
                for step in tqdm(range(phase_config["num_steps"]), leave=False, colour="red"):
                    ...
                    loss, model = optimize_with_bfgs(
                        ...
                        model=model, nn_off=phase_config["nn_off"], filter_spec=filter_spec
                    )
                    # res = minimize(
                    #     fun=lambda x: mse(
                    #         model, rho=rho, ts=ts, gt=gt_fid.real[:length], nn_off=phase_config["nn_off"]
                    #     ),
                    #     x0=model.h_mat.ravel(),
                    #     method="BFGS",
                    #     jac=lambda x: updates.h_mat.ravel()
                    # )
        ...
        jnp.savez(f"{path.stem}_HL-BFGS-SD={sd}-Fitted.npz", ppm=final_ppm, spec=final_FTspec, fid=final_fid)
        jnp.savez(f"{path.stem}_HL-BFGS-SD={sd}-GT.npz", ppm=gt_ppm, spec=gt_FTspec, fid=gt_fid)
        jnp.savez(f"{path.stem}_HL-BFGS-SD={sd}-Initial.npz", ppm=init_ppm, spec=init_FTspec, fid=init_fid)
