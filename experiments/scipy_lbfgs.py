...
from stNMR.numpy.nmr import generate_spin_operators, calc_hamiltonian


def make_hamiltonian(v: np.ndarray, J: np.ndarray, Ix: np.ndarray, Iy: np.ndarray, Iz: np.ndarray, B0: int) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    initial_guess = np.random.rand(n_spins * 2,)
    options = {"disp": False, "gtol": 1e-8, "maxiter": 1000, "ftol":1e-8}
    ...
    for n in range(N):
        ...
        result = minimize(objective_function, curr_params, method="L-BFGS-B", args=(length,), options=options)
        curr_params = result.x
    ...
    logger.info("="*50)
    logger.info(f"GT values:\n{gt_h_mat}")
    logger.info(f"Initial guess: {initial_guess}")
    logger.info(f"Fianl Fitted value: {curr_params}")
